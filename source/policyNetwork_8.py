...
def cnn_model_fn(features,labels,mode):
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=0.0001,beta1=0.88,beta2=0.995,epsilon=1e-07)
...
def main(unused_argv):
    ...
    train_data_features = data_features[:int(0.95*num_data)]
    test_data_features = data_features[int(0.95*num_data):]
    train_data_labels = data_labels[:int(0.95*num_data)]
    test_data_labels = data_labels[int(0.95*num_data):]

    #create estimator
    cnn_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn,
        model_dir="../model/policynetwork_eight_layer_alldata_dropout"
        )
    #create log
